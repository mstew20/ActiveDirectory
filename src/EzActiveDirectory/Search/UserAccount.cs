...
public class UserAccount
{
	...
	public bool EnableAccount(string path, UserCredentials credentials = null)
	{
		var output = false;
		try
		{
			using var directoryEntry = _ad.GetDirectoryEntry(path, credentials);
			directoryEntry.Properties[Property.AccountControl].Value = AccountFlag.Normal;
			directoryEntry.CommitChanges();
			output = true;
		}
		catch { }
	...
	public bool DisableAccount(string path, UserCredentials credentials = null)
	{
		var output = false;
		try
		{
			using var directoryEntry = _ad.GetDirectoryEntry(path, credentials);
			directoryEntry.Properties[Property.AccountControl].Value = AccountFlag.Disable;
			directoryEntry.CommitChanges();
			output = true;
		}
		catch { }
	...
	public List<ActiveDirectoryGroup> GetGroups(string userPath, UserCredentials credentials = null)
	{
		...
		foreach (var g in user.Properties[Property.GroupMember])
		{
			...
			groupPath = $"{userPath[..userPath.LastIndexOf('/')]}/{groupPath}";
	...
	private List<ActiveDirectoryUser> ConvertToActiveDirectoryUser(List<UserResultCollection> users)
	{
		...
		if (_ad.MaxPasswordAge == 0)
		{
			_ad.MaxPasswordAge = _ad.GetMaxPasswordAge();
		}
		...
		foreach (var userResults in users)
		{
			ActiveDirectoryUser user = new()
			{
				...
				MaxPasswordAge = _ad.MaxPasswordAge
			};
